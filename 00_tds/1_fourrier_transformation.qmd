...
```{python}
import numpy as np
from scipy.fft import fft, fftshift, fftfreq, ifftshift

# Function to compute the Fourier Transform using FFT using SciPy

def fourier_at_nu_scipy(f, T, n, nu):
    """
    Computes the Fourier transform of f at frequency nu
    using scipy's FFT + Shannon interpolation.
    """
    t = np.linspace(-T/2, T/2, n, endpoint=False)
    F = f(t)
    tf = fftshift(fft(ifftshift(F))) * (T/n)
    return shannon(tf, nu, T)
...
fhat_exact = lambda nu: (2 * a) / (a**2 + 4 * np.pi**2 * nu**2)  # exact TF
T = 40     # window width
n = 2**10  # number of discretization points
# Calculation for nu = 3/T
nu = 3 / T
# Exact value of the TF
exact_value = fhat_exact(nu)
# Approximation de la TF en nu
approx_value_scipy = fourier_at_nu_scipy(f, T, n
, nu)
print(f"Exact value at nu={nu}: {exact_value}")
print(f"Approximation (SciPy) at nu={nu}: {np.real(approx_value_scipy)}")

# Calculation for nu = pi/T
nu = np.pi / T
# Exact value of the TF
exact_value = fhat_exact(nu)
# Approximation de la TF en nu
approx_value_scipy = fourier_at_nu_scipy(f, T, n, nu)
print(f"Exact value at nu={nu}: {exact_value}")
print(f"Approximation (SciPy) at nu={nu}: {np.real(approx_value_scipy)}")
```
...
np.random.seed(42)
n_steps = 80
reference = np.random.normal(5, 1, 2000)
threshold = 0.6

# Données avec dérive progressive
production_data = [np.random.normal(5 + 0.03*t, 1, 2000) for t in range(n_steps)]

# Calcul des distances Jensen-Shannon
bins = np.linspace(0, 12, 60)
ref_hist, _ = np.histogram(reference, bins=bins, density=True)
js_distances = []
for prod in production_data:
    prod_hist, _ = np.histogram(prod, bins=bins, density=True)
    js = jensenshannon(ref_hist, prod_hist)
...
sns.kdeplot(reference, ax=ax1, fill=True, color='blue', alpha=0.5, label='Reference (Train) Data')
line_prod, = ax1.plot([], [], color='teal', lw=2, label='Production Data')

ax1.set_xlim(0, 12)
ax1.set_ylim(0, 0.8)
ax1.set_title("Density of Feature Distributions")
ax1.set_xlabel("Feature Value")
ax1.set_ylabel("Density")
ax1.legend()

line_js, = ax2.plot([], [], color='blue', label='Jensen-Shannon Distance')
threshold_line = ax2.axhline(threshold, color='black', linestyle='--', label=f'Threshold = {threshold}')
ax2.set_xlim(0, n_steps)
ax2.set_ylim(0, 1)
ax2.set_title("Distance Metrics Over Time")
...
def init():
    line_prod.set_data([], [])
    line_js.set_data([], [])
    return line_prod, line_js

def update(frame):
    # Production distribution
    prod = production_data[frame]
    prod_hist, _ = np.histogram(prod, bins=bins, density=True)
    line_prod.set_data(x_centers, prod_hist)

    # JS distance curve
    line_js.set_data(np.arange(frame+1), js_distances[:frame+1])

    # Couleur rouge si dérive détectée
    if js_distances[frame] > threshold:
        line_js.set_color('red')
    else:
        line_js.set_color('blue')
...
anim.save("data_drift.gif", writer=gif_writer)

print("✅ GIF enregistré : data_drift.gif")
...
# Paramètres
# -------------------------------
N = 5000      # entiers possibles
M = 50000     # nombre de simulations
p_theorique = 0.4  # valeur calculée théoriquement

# -------------------------------
# Simulation Monte Carlo
# -------------------------------
# Tirages aléatoires uniformes entre 1 et 5000
tirages = np.random.randint(1, N+1, size=M)

# Événement : divisible par 4, 7 ou 10
succes = (tirages % 4 == 0) | (tirages % 7 == 0) | (tirages % 10 == 0)

# Moyenne cumulative
proba_cumulative = np.cumsum(succes) / np.arange(1, M+1)

# -------------------------------
# Tracé
# -------------------------------
plt.figure(figsize=(10,5))
plt.plot(np.arange(1, M+1), proba_cumulative, color='blue', linewidth=2, label='Estimation cumulative')
plt.axhline(y=p_theorique, color='red', linestyle='--', linewidth=2, label='Valeur théorique = 0.4')

plt.xlabel("Nombre de simulations (M)")
plt.ylabel("Probabilité estimée")
plt.title("Convergence de la simulation de Monte Carlo")
plt.legend()
